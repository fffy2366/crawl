...
dbname = './database.db'
# 设置提交的header头
headers = {"Accept": "*/*", "Referer": "http://www.liangcuntu.com/",
           "User-Agent": "Mozilla/5.0+(compatible;+Googlebot/2.1;++http://www.google.com/bot.html)"}
# 连接服务器
dl = httplib.HTTPConnection('www.liangcuntu.com')
# 连接数据库
...
def createDatabase():
    ...
    # 创建url列表存放表
    c.execute(
        '''CREATE TABLE IF NOT EXISTS [collect]([cid] INTEGER PRIMARY KEY,[curl] TEXT,[state] INTEGER DEFAULT '0',UNIQUE([curl]));''')
    c.execute('''CREATE INDEX IF NOT EXISTS [collect_idx_state] ON [collect]([state]);''')
    # 创建分类表
    c.execute(
        '''CREATE TABLE IF NOT EXISTS [sorts]([sortid] INTEGER PRIMARY KEY,[sortname] TEXT,[sortpath] TEXT,[sortfoot] INTEGER DEFAULT '0',[sortnum] INTEGER DEFAULT '0',UNIQUE([sortpath]));''')
    ...
    # 创建文章表
    c.execute(
        '''CREATE TABLE IF NOT EXISTS [article]([aid] INTEGER PRIMARY KEY,[sortid] INTEGER DEFAULT '0',[hits] INTEGER DEFAULT '0',[title] TEXT,[path] TEXT,[question] TEXT,[banswer] TEXT,[oanswer] TEXT,UNIQUE([path]));''')
...
# 执行采集
def collect(url="http://www.liangcuntu.com/"):
    global dl, error, headers;
    R = 0
    print "GET:", url
    urls = urlparse(url);
    path = urls[2];
    ...
    if re.search('http(.*)/question/index(.*)', url, re.I) != None:
        ...
        if aGuide != None:
            ...
            for sortname in aGuide:
                ...
                # 分类不存在，添加分类
                if row == None:
                    c.execute('INSERT INTO [sorts]([sortname],[sortpath],[sortfoot])VALUES(?,?,?);',
                              (sortname, sortpath, sortfoot))
            ...
            if BestAnswer != None:
                ...
                if title != None and content != None:
                    ...
                    try:
                        c.execute(
                            'INSERT INTO [article]([sortid],[title],[path],[question],[banswer],[oanswer])VALUES(?,?,?,?,?,?);',
                            (sortfoot, title, path, content, BestAnswer, OtherAnswer))
